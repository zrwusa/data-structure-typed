 ...
 *     const evens = tm.filter((v, k) => k % 2 === 0);
 ...
 *     const sum = tm.reduce((acc, v) => acc + (v ?? 0), 0);
 *     console.log(sum); // 60;
   */
   ...
   * @remarks Time O(n), Space O(1)
    ...
    * @example
 * // Test all entries
 *  const tm = new TreeMap<number>([2, 4, 6]);
 ...
 *     console.log(tm.some((v, k) => k === 3)); // true;
   */
