...
import { compileCppBenchmarks, discoverCppBenchmarks, runCppBenchmark } from './cmake-benchmark-runner.mjs';
...
function printFileLoading(fileName, index, total, type) {
  ...
  console.log(`  ${progressBar} ${CYAN}↳${END} ${GRAY}[${index}/${total}]${END} ${type}: ${BOLD}${fileName}${END}`);
}
...
function printFileError(fileName, index, total, error) {
  console.log(`  ${RED}✗${END} ${GRAY}[${index}/${total}]${END} ${fileName} - ${YELLOW}${error}${END}`);
}
    ...
    try {
      if (fs.existsSync(p)) {
        ...
        if (Array.isArray(data)) {
          ...
          if (Array.isArray(data.order)) cfg.order = data.order.filter(x => typeof x === 'string');
          if (Array.isArray(data.include)) cfg.include = data.include.filter(x => typeof x === 'string');
          if (Array.isArray(data.exclude)) cfg.exclude = data.exclude.filter(x => typeof x === 'string');
...
function matchesGlobPattern(filePath, pattern) {
  ...
  return regex.test(fileName) || regex.test(relPath) || testName === pattern || testName.includes(pattern);
}
...
function matchesFilters(file, filterList) {
  ...
  return filterList.some(word => fileName.includes(word) || file.includes(word));
}
...
function applyConfigRules(files, config) {
  ...
  if (config.include && config.include.length > 0) {
    filtered = filtered.filter(file => config.include.some(pattern => matchesGlobPattern(file, pattern)));
  }

  // Apply exclude patterns
  if (config.exclude && config.exclude.length > 0) {
    filtered = filtered.filter(file => !config.exclude.some(pattern => matchesGlobPattern(file, pattern)));
  }

  // Apply order
  if (config.order && config.order.length > 0) {
    filtered.sort((a, b) => {
      const nameA = path.basename(a, path.extname(a)).replace(/\.test$/, '');
      const nameB = path.basename(b, path.extname(b)).replace(/\.test$/, '');
  ...
  if (config.include && config.include.length > 0) {
    filtered = filtered.filter(file => {
      return config.include.some(pattern => {
        ...
        if (pattern.endsWith('.test.mjs') || pattern.endsWith('.test.ts')) {
...
const allJsFiles = !flags.cppOnly
  ? fastGlob.sync([path.join(testDir, '**', '*.test.ts'), path.join(testDir, '**', '*.test.mjs')])
  : [];
...
let allCppFiles = !flags.jsOnly ? fastGlob.sync([path.join(testDir, '**', '*.test.cpp')]) : [];
...
function loadPartialResults() {
  try {
    if (fs.existsSync(PARTIAL_RESULTS_FILE)) {
      ...
      console.log(
        `${CYAN}ℹ Loaded ${data.javascript?.length || 0} JS + ${data.cpp?.length || 0} C++ partial results${END}`
      );
      return data;
    }
  } catch (e) {
...
async function generateUnifiedReport(jsResults, cppResults) {
  ...
  if (fs.existsSync(filePath) && !flags.fullFresh) {
    try {
      ...
      console.log(
        `${CYAN}ℹ Merging with existing report (${existingReport.javascript?.length || 0} JS, ${existingReport.native?.length || 0} C++ tests)${END}`
      );
    } catch (e) {
  ...
  if (jsResults.length < mergedJsResults.length || cppResults.length < mergedCppResults.length) {
    console.log(
      `${CYAN}ℹ Incremental merge: updated ${jsResults.length} JS + ${cppResults.length} C++ tests, total now ${mergedJsResults.length} JS + ${mergedCppResults.length} C++${END}`
    );
  }
...
async function main() {
  ...
  if (!flags.cppOnly && jsTestFiles.length > 0) {
    ...
    for (let i = 0; i < jsTestFiles.length; i++) {
      ...
      const testName = path.basename(file, path.extname(file)).replace(/\.test$/, '');
      ...
      try {
        ...
        if (testModule.results || testModule.getResults) {
          ...
          const raw = testModule.getResults ? await testModule.getResults() : await Promise.resolve(testModule.results);
              ...
              .on('complete', function () {
                ...
                benchmarks.push(
                  ...this.map(benchmark => {
                    ...
                    const scale =
                      typeof scaleOps === 'number' && Number.isFinite(scaleOps) && scaleOps > 0 ? scaleOps : 1;
