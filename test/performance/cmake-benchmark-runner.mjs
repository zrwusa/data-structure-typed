  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (
        ...
        line.includes('Run on') ||
        line.includes('Caches')
      ) {
        continue;
      }
      ...
      const match = line.match(/^(.+?)\s+([\d.]+)\s+(ns|us|ms|s)\s+([\d.]+)\s+(ns|us|ms|s)\s+(\d+)\s*$/);
...
export function discoverCppBenchmarks(testFiles = []) {
  ...
  if (testFiles && testFiles.length > 0) {
    for (const testFile of testFiles) {
      ...
      const patterns = [`${testName}-benchmark`, `${testName}_benchmark`, testName];
