...
export async function getResults() {
  ...
  console.log(
    `RBT macro perf (ESM): N=${N.toLocaleString()} KEY_POOL=${KEY_POOL.toLocaleString()} GET_N=${GET_N.toLocaleString()} REPEATS=${REPEATS}`
  );
  ...
  console.log(
    `pre-building rangeSearch tree + queries (not timed)... QUERY_COUNT=${QUERY_COUNT.toLocaleString()} RANGE_SIZE=${RANGE_SIZE}`
  );
  ...
  results.push(
    withRepeats(`${GET_N.toLocaleString()} get`, () => {
      let count = 0;
  ...
  results.push(
    withRepeats(`${GET_N.toLocaleString()} get (DST classic)`, () => {
      let count = 0;
  ...
  results.push(
    withRepeats(`${QUERY_COUNT.toLocaleString()} rangeSearch SEQ`, () => {
      let totalResults = 0;
  ...
  results.push(
    withRepeats(`${QUERY_COUNT.toLocaleString()} rangeSearch RAND`, () => {
      let totalResults = 0;
      for (let i = 0; i < QUERY_COUNT; i++) {
  ...
  // UPDATE (SEQ)
  results.push(
    withRepeats(`${MILLION.toLocaleString()} upd SEQ`, () => {
  ...
  results.push(
    withRepeats(`${MILLION.toLocaleString()} upd SEQ (DST classic)`, () => {
  ...
  // UPDATE (RAND)
  results.push(
    withRepeats(`${MILLION.toLocaleString()} upd RAND`, () => {
      const rbTree = new RedBlackTree([], { isMapMode: true });
      for (let i = 0; i < KEY_POOL; i++) rbTree.set(i, 0);
  ...
  results.push(
    withRepeats(`${MILLION.toLocaleString()} upd RAND (DST classic)`, () => {
      const rbTree = new RedBlackTree([], { isMapMode: false });
      for (let i = 0; i < KEY_POOL; i++) rbTree.set(i, 0);
      for (let i = 0; i < N; i++) rbTree.set(randUpdateKeys[i], i);
    })
  ...
  // INSERT (SEQ)
  results.push(
    withRepeats(`${N.toLocaleString()} ins SEQ`, () => {
      const rbTree = new RedBlackTree([], { isMapMode: true });
  ...
  results.push(
    withRepeats(`${MILLION.toLocaleString()} ins SEQ (DST classic)`, () => {
      const rbTree = new RedBlackTree([], { isMapMode: false });
  ...
  // INSERT (RAND unique)
  results.push(
    withRepeats(`${MILLION.toLocaleString()} ins RAND`, () => {
      const rbTree = new RedBlackTree([], { isMapMode: true });
      for (let i = 0; i < N; i++) {
  ...
  results.push(
    withRepeats(`${MILLION.toLocaleString()} ins RAND (DST classic)`, () => {
      const rbTree = new RedBlackTree([], { isMapMode: false });
  ...
  // Baseline: keys-only
  results.push(
    withRepeats(`${MILLION.toLocaleString()} keys-only`, () => {
      let s = 0;
