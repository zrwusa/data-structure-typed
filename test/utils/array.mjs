import { getRandomInt } from './number.mjs';
...
export function getRandomWords(length, distinct = true) {
  if (length <= 0) {
  ...
  const randomWords = [];
  while (randomWordsSet.size < length) {
    const randomIndex = Math.floor(Math.random() * words.length);
    const randomWord = words[randomIndex];
    if (!randomWordsSet.has(randomWord)) {
...
export function getRandomIndex(vertexes) {
  return getRandomInt(0, vertexes.length - 1);
}
