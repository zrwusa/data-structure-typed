import { isDebugTest } from '../config.mjs';
...
function estimateBigO(runtimes, dataSizes) {
  ...
  for (const complexity of complexitiesToCheck) {
    ...
    const fittedData = dataSizes.map(size => {
      if (complexity === 'O(1)') {
...
const methodLogs = new Map();
export function logBigOMetricsWrap(fn, args, fnName) {
...
export function logBigOMetrics(target, propertyKey, descriptor) {
...
export const logPerf = function (label = 'function running cost', fn, args, thisArg) {
